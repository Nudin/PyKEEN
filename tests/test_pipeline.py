...
class TestPipeline(unittest.TestCase):
    ...
    def test_run(self):
        ...
        config_path = os.path.join(
            RESOURCES_DIRECTORY, "configuration_training_without_eval.json"
        )
        ...
        config["training_set_path"] = os.path.join(
            RESOURCES_DIRECTORY, "example_training.tsv"
        )

        config["test_set_path"] = os.path.join(RESOURCES_DIRECTORY, "example_test.tsv")

        self.p.config = config
        results = self.p.run()
        self.assertIsNotNone(results)

        config_path = os.path.join(RESOURCES_DIRECTORY, "configuration_hpo.json")
