...
class ConvE(BaseModule):
    ...
    def forward_conv(self, subject, relation):
        ...
        subject_batch_embedded = self.entity_embeddings(subject).view(
            -1, 1, self.ConvE_height, self.ConvE_width
        )
        relation_batch_embedded = self.relation_embeddings(relation).view(
            -1, 1, self.ConvE_height, self.ConvE_width
        )
    ...
    def predict_objects(self, subject, relation):
        ...
        scores = (
            self.predict_for_ranking(
                torch.tensor([subject], device=self.device),
                torch.tensor([relation], device=self.device),
            )
            .detach()
            .cpu()
            .numpy()
    ...
    def predict_subjects(self, object, relation):
        ...
        scores = (
            self.predict_for_ranking(
                torch.tensor([object], device=self.device),
                torch.tensor([relation], device=self.device),
            )
            .detach()
            .cpu()
    ...
    def score_triple(self, triple):
        ...
        x = torch.sum(
            torch.mul(x.flatten(), candidate_object_embeddings.flatten()).reshape(
                batch_size, -1
            ),
            dim=1,
        )
    ...
    def forward(self, pos_batch, labels):
        ...
        if self.label_smoothing is not None:
            labels_full = (
                labels_full * (1 - self.label_smoothing)
