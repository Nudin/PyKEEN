...
class BaseModule(nn.Module):
    ...
    def load_triples_from_path(self, data_paths: Union[str, List[str]]):
        """
    ...
    def map_triples_from_path(self, data_paths: Union[str, List[str]]):
        """
    ...
    def predict_objects(self, subject: str, relation: str) -> str:
        """"""
        ...
        scored_objects = np.vstack(
            (object_values[sorting[::-1]], scores[sorting[::-1]])
        )

        return scored_objects.T

    def predict_best_object(self, subject: str, relation: str) -> str:
        """"""
        return self.predict_objects(subject, relation)[0]

    def predict_subjects(self, obj: str, relation: str) -> str:
        """"""
        ...
        scored_subjects = np.vstack(
            (subject_values[sorting[::-1]], scores[sorting[::-1]])
        )

        return scored_subjects.T

    def predict_best_subject(self, obj: str, relation: str) -> str:
        """"""
    ...
    def fit(
        ...
        log.info(
            f"Created inverse triples. It took {timeit.default_timer() - start:.2f} seconds"
        )
        ...
        for _ in trange_bar:
            ...
            labels_batches = _split_list_in_batches(
                input_list=labels_list, batch_size=num_positives
            )
            # pos_triples_tensor = torch.tensor(pos_triples, device=self.device)
            current_epoch_loss = 0.0

            for i, (pos_batch, labels_batch) in enumerate(
                zip(pos_batches, labels_batches)
            ):
            ...
            epoch_loss = current_epoch_loss / (
                len(pos_triples) * self.entity_embeddings.num_embeddings
            )
